...
- **Bloque CSS `funnel`/`orig-chart` duplicado 4× — candidato a centralizar (C2.1-bis, NO ahora):** el bloque de embudo (`.fstep/.fbar/.fbar-wrap/.fnum/.feq/.bar-c/.bar-p/.orig-*`, verbatim del legacy plataforma) está repetido en `simulador/page.tsx`, `simulador-consorcio/page.tsx`, `tracker-consorcio/page.tsx` y `plataforma/page.tsx`. Se descubrió porque el simulador Zurich lo **omitía** (hueco del port Sección 3, restituido en commit `e83ec04`): su embudo solo se veía estilado cuando un `<style>` de otra página `/app` quedaba en el DOM. Igual que el CSS shell, es candidato a extraer a una fuente única (paralelo a `SHELL_CSS`/C2.1). **NO ahora:** se hace después de terminar la migración de páginas C2, como **C2.1-bis** (mismo criterio que Nivel B del bloque de los simuladores). Hasta entonces, al migrar/portar una página con embudo, verificar que su bloque funnel esté completo en su propio `<style>`.
- **Override responsive `@media .fstep` ausente en simulador Zurich (preexistente):** el `@media(max-width)` de `simulador-consorcio` y `tracker-consorcio` incluye `.fstep{grid-template-columns:90px 1fr 44px 90px;gap:8px}`, pero el de `simulador/page.tsx` (Zurich) **no** lo tiene (el embudo Zurich no compacta su grilla en móvil). No es parte del bloque funnel contiguo (vive en el `@media` general) y NO se tocó en C2.1-bis (refactor puro). Igualar si/cuando se aborde el frente Zurich (login-Zurich).
- **module-bar duplicado por-página en `/app` — candidato a centralizar (NO ahora):** el module-bar ("📊 Simulador" / "📋 Tracker") está duplicado por-página en las rutas `/app`; candidato a moverse al shell/layout unificado con los módulos visibles según rol (idea original C2 / P4). Lo funcional ya está cubierto por el gate de rol de C1.2 (un asesor que entra a simulador-consorcio/tracker-consorcio es redirigido a `/app/informe`), así que esto es solo consolidación de UI, no un fix. Abordar en Fase 2/3 (mismo criterio que las otras deudas de shell `/app`).
- **Producto `PM` ("Vida Mujer") en la suite golden pero NO en el simulador — confirmar con contrato (pendiente):** el producto `PM` existe en el modelo de la suite golden (`tools/compensacion-golden/golden-master.js` y `zurich-spec.js`, 8 productos VI), pero **no está expuesto en el simulador legacy** (`compania-z/datos.js`, 7 productos, sin PM) **ni en el port React** (`src/lib/simulador/calculo.ts`, mismo set de 7). Detectado al comparar legacy↔React (paridad 68/68): los escenarios golden que setean `PM` lo cuentan en la referencia, pero ni legacy ni React lo procesan (no está en sus `SIM_PRODS`), así que es inerte e idéntico en ambos. No es divergencia legacy↔React; es una diferencia entre la suite golden y el código en producción. **Pendiente: confirmar con el contrato de Zurich si `PM` debe incorporarse al simulador** (y, si sí, alinear datos.js + calculo.ts) o si debe quitarse de la spec/suite golden.
