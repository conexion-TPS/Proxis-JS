...
Contexto: Viña vive en su PROPIO proyecto Supabase (sgu-vina-prospección), totalmente separado de Zurich (proxis-dev) y del proyecto de la app (B). Login DB (bcrypt+JWT) contra vina_credentials. Acceso solo vía API routes con service-role; RLS cerrado. La columna `empresa` queda en 'vina' (residual, por no cambiar el código). **[Corregido 2026-06-15, verificado contra BD: (1) el RLS NO estaba cerrado — las tablas tenían `acceso_total` abierto a public; recién el 06-15 se cerró + se revocó la escritura (lectura-sólo). (2) `empresa` NO quedó en 'vina': se reclasificó a 'consorcio' y se agregó 'zurich' (Fase 0.1 + alta Zurich); valores reales = zurich/consorcio. (3) Matiz: las credenciales de login Zurich viven en `vina_credentials` (proyecto Viña), no solo en proxis-dev.]**
...
6. Migrar login de Zurich a DB: Zurich sigue con la lista hardcodeada USUARIOS en plataforma-core.js. PENDIENTE. **[Corregido 2026-06-15: el login Zurich YA está en BD (bcrypt en `vina_credentials`, alta 12-jun). Lo pendiente es ELIMINAR el `USUARIOS` hardcodeado (rotación D-LZ5 + apagado legacy, ya redirigido).]**
