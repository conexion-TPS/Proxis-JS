...
/*
 * Cálculo de "Mi Informe" — extraído 1:1 de src/app/api/app/informe/route.ts (T3a).
 * Lee de proxis_dev por persona_id. KPIs calculados server-side (portado fiel de
 * calcIndicadores() del legacy). Compartido por /api/app/informe y /api/app/individual.
 * Transcripción literal: misma lógica, mismos números, misma estructura de salida.
 */
...
export type Identidad = { nombre: string; institucion: string | null; via: string; tipo: string }
...
// ── Construye el payload de Mi Informe para un persona_id + mes. La `identidad` se inyecta
//    en su posición original (mismo orden de claves que devolvía el route inline) para que
//    /api/app/informe quede byte-idéntico. ──
export async function buildInforme(sb: SB, persona_id: string, mes: string, identidad: Identidad) {
  ...
  if (reportes.length === 0) {
    return { mes, hasReportes: false, semanasCount: 0, identidad }
  }
