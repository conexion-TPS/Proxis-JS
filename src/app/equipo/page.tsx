...
import Desempeno, { PIE_INFORME } from './Desempeno'
...
export default function EquipoDashboard() {
  ...
  // Pestaña activa del módulo Desempeño (la informa Desempeno.tsx vía onVistaChange):
  // en "Detalle por asesor" ocultamos la lista legada + la miga de pan.
  const [desempenoVista, setDesempenoVista] = useState<'gen' | 'det'>('gen')
      ...
      <div style={{ maxWidth: hasTree ? 1180 : 900, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: hasTree ? 'grid' : 'block', gridTemplateColumns: hasTree ? '320px 1fr' : undefined, gap: 24, alignItems: 'start' }}>
          ...
          <div>
            ...
            <Desempeno token={token} onVistaChange={setDesempenoVista} />
            {/* Lista legada + miga "Viendo:" + footer: visibles solo en "Vista general"; ocultos en "Detalle por asesor" */}
            {desempenoVista !== 'det' && (<div>
            {hasTree && (
            ...
            {/* Pie legal de la vista supervisora (Vista general) */}
            <div style={{ textAlign: 'center', fontSize: 10.5, color: '#b6b3ae', padding: '18px 0 8px' }}>{PIE_INFORME}</div>
            </div>)}
          </div>
        </div>
      </div>
