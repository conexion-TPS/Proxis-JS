/* ═══════════════════════════════════════════════════════════════════════
   SPEC DECLARATIVA de Zurich (compania-z) como DATOS. UMD: navegador + Node.
   Fuente única del modelo de pago de Zurich para el MOTOR genérico.
   Espejo de tools/compensacion-golden/zurich-spec.js (validado 36/36).
   Futuro: migrar a la tabla comp_specs en Supabase (la gestionará el admin).
═══════════════════════════════════════════════════════════════════════ */
'use strict'
;(function (root) {
  const SPEC = {
    ...
    productosGI: [ { id:'AUTO', factor:0.50 }, { id:'HOGAR', factor:1.00 } ],
    aportes: [
    ...
    campania: {
      productoKPI: 'APV', kpiSaludProductoId: 'SS',
      overrides: {
    ...
    reglas: { campaniaModo: 'reemplaza', topeAntiguedadAplicaA: 'tramo5' },
  }

  if (typeof module !== 'undefined' && module.exports) module.exports = SPEC
  if (root) root.COMP_SPEC_ZURICH = SPEC
